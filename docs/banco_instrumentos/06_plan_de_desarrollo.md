...
> **Avance (2026-09-19):** Fases 1, 2, 3 y 4 implementadas y documentadas en un mismo
> entregable. Código en `jornadas/` (modelo, migraciones 0016/0017, `banco.py`, serializers,
> `BancoMomentoViewSet`, `tests_banco.py` con 55 tests), HU-67 a HU-70 en
> `docs/USER_STORIES_COMPLETO.md` y guía `docs/INTEGRACION_FRONTEND_BANCO_INSTRUMENTOS.md`.
> Pendiente: despliegue (`migrate`) y validación del flujo con el equipo de frontend.
...
Tareas:
- [x] `jornadas/models.py`: campos `visibilidad`, `creado_por`, `momento_origen`,
      `origen_info` en `Momento` (ver [03_modelo_de_datos.md](03_modelo_de_datos.md) §1).
- [x] `jornadas/migrations/0016_momento_banco.py` (esquema) y
      `0017_momento_backfill_creado_por.py` (datos, D7).
- [x] `jornadas/serializers.py` → `MomentoAdminSerializer`: `visibilidad` escribible;
      `creado_por` (nested `{id, username, nombre}`), `momento_origen`, `origen_info` read-only.
- [x] `jornadas/views.py` → `MomentoAdminViewSet.perform_create`: fijar
      `creado_por=request.user`. Filtro opcional `?visibilidad=`.
- [x] `jornadas/admin.py`: mostrar los campos nuevos en el admin de Django (si hay `ModelAdmin`
      de Momento).
- [x] HU-67 en `docs/USER_STORIES_COMPLETO.md`.
...
Tareas:
- [x] `jornadas/banco.py`: `copiar_momento(origen, jornada_destino, usuario, *, orden, titulo,
...
- [x] Función auxiliar `siguiente_orden(jornada)` con `select_for_update`.
- [x] Función auxiliar `advertencias_roles(momento_copia, jornada_destino)`.
- [x] HU-68 en `docs/USER_STORIES_COMPLETO.md` (explica por qué copia profunda y no referencia,
...
Tareas:
- [x] `jornadas/serializers.py`: `BancoMomentoListaSerializer` (item del listado con
      ...
      `usar/`).
- [x] `jornadas/scoping.py`: `momentos_del_banco(user)` con el queryset de
...
- [x] `jornadas/views.py`: `BancoMomentoViewSet(ReadOnlyModelViewSet)` con filtros `alcance`,
      `q`, `tipo`, `jornada`, `solo_originales`, `incluir_inactivos` (solo en `list`),
      `ordering`; acciones `usar` (POST) y `derivados` (GET).
- [x] `jornadas/urls.py`: `router.register('banco-momentos', BancoMomentoViewSet,
      basename='banco-momento')`.
- [x] `@extend_schema` para el listado y `usar/`.
- [x] HU-69 en `docs/USER_STORIES_COMPLETO.md`.
...
## Fase 4 — Documentación para el frontend y cierre (½ día) → **HU-70** ✅ (salvo despliegue y validación con FE)

- [x] `docs/INTEGRACION_FRONTEND_BANCO_INSTRUMENTOS.md`: pantallas sugeridas (selector
...
- [x] HU-70: trazabilidad (`derivados/`, `origen_info`) si no cupo en HU-69.
- [x] Actualizar `README.md` del repo (sección de endpoints) y `CLAUDE.md` si describe el módulo
      de jornadas.
- [x] Correr la suite completa una sola vez (`python manage.py test`), según la regla de ritmo
      del proyecto.
- [x] Commit y push directo a `main`.
- [ ] Despliegue de producción (`migrate` obligatorio por las migraciones 0016/0017).
