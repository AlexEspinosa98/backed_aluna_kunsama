...
- **Tres papeles fijos, enunciados en el prompt**: portada (nombre de la jornada + cifras de participación), hallazgos (los temas con su dato y las visualizaciones) y cierre (conclusiones accionables). El prompt dice además qué **no** debe repetirse entre ellas, porque sin esa instrucción el modelo vuelve a meter el título grande en las tres.
- **Una sola llamada con `n=3`, pedida como SERIE.** `n` no acepta un prompt por imagen, pero gpt-image-2 razona sobre el conjunto cuando el prompt lo describe como una secuencia/storyboard — ese es el mecanismo documentado para obtener un set coherente y diferenciado, y evita triplicar las llamadas. Se probó antes una llamada por lámina en paralelo; funciona, pero es innecesario y este modelo no lo requiere.
- **Consistencia visual explícita**: una instrucción de que las tres pertenecen a la misma serie y comparten paleta, tipografía y lenguaje visual. Sin eso se veían como piezas de autores distintos.
