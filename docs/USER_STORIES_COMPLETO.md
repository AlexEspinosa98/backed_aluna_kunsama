...
- **Una llamada por lámina, no `n=3` en una sola.** Esa era la causa real de la redundancia: pedir tres imágenes en la misma llamada devuelve tres *variaciones* del mismo encargo, que es justo lo contrario de lo que sirve. Cada lámina tiene ahora su propio prompt.
- **Tres papeles fijos** (`SLIDES`): portada (nombre de la jornada + cifras de participación), hallazgos (los temas con su dato y las visualizaciones) y cierre (conclusiones accionables). Cada prompt dice además qué **no** repetir de las otras, porque sin esa instrucción el modelo tiende a volver a meter el título grande en las tres.
...
- **16:9 real en píxeles** (`1920x1080`, configurable con `OPENAI_IMAGE_SIZE`). El SDK declara un enum cerrado de tamaños (1024x1024 / 1536x1024 / 1024x1536) donde el apaisado es 3:2, pero el modelo acepta cualquier resolución, así que se pide la medida exacta. Se descartó recortar con Pillow: mutilar los bordes de una lámina con texto es peor que pedir el lienzo correcto de entrada.
- **Una lámina que falla ya no tira las otras dos.** Cada una es una llamada pagada aparte; si salen 1 o 2, se guardan y el estado queda `completo` con un `error_mensaje` que dice cuál faltó. Solo si fallan las tres el estado es `error`.
