...
| Rediseño (endpoints existentes → v2) | `f87afb7` | 2026-09-20 | Corrección de rumbo del dueño del repo tras la fase 7: el contrato v2 no debía quedar como una cuarta vía aparte (`analisis-v2/`), sino REEMPLAZAR la salida de los tres endpoints que el frontend ya integraba. `analizar_jornada_ia`/`analizar_momento_ia` (`analisis_ia_openai.py`) y `procesar_reporte` (`analysis.py`) ahora corren `analitica/v2/procesar.py::ejecutar_analisis_v2` y guardan el JSON v2 en `resultado`/`analisis`; `Reporte`/`AnalisisMomentoIA`/`AnalisisJornadaIA` ganan `entrada`/`diagnostico`/`version_prompt`/`version_esquema` (mixin `ResultadoV2Mixin`, migración `0019`); la lista unificada expone `version`/`estado_analitico` en todos los items; la infografía lee v2 desde las cuatro fuentes; presentación HTML y PDF responden `400` claro para un `Reporte` v2. `enfoque`/`plantilla` se siguen aceptando por compatibilidad pero ya no influyen. `analisis-v2/` (fases 0–7 de esta tabla) sigue existiendo tal cual, como vía adicional con la misma salida. Documentado como HU-78 en `docs/USER_STORIES_COMPLETO.md`; guía de integración actualizada en `docs/INTEGRACION_FRONTEND_ANALISIS_V2.md` §1. |
